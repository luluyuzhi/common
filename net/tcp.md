...
![Linux TCP 参数](../source/img/2.jpg)Linux TCP 参数
...
TCP 是面向**连接的、可靠的、双向传输**的传输层通信协议，所以在传输数据之前需要经过三次握手才能建立连接。

![三次握手与数据传输](../source/img/4.jpg)
...
![TCP 三次握手的状态变迁](../source/img/5.jpg)TCP 三次握手的状态变迁
...
![TCP 头部](../source/img/6.jpg)TCP 头部
...
![半连接队列与全连接队列](../source/img/9.jpg)半连接队列与全连接队列
...
![img](../source/img/11.jpg)

增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：

![img](../source/img/12.jpg)
...
![开启 syncookies 功能](../source/img/13.jpg)开启 syncookies 功能
...
![tcp_abort_on_overflow 为 0 可以应对突发流量](../source/img/18.jpg)tcp_abort_on_overflow 为 0 可以应对突发流量
...
![常规 HTTP 请求](../source/img/21.jpg)常规 HTTP 请求

在 Linux 3.7 内核版本之后**，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。**

> 接下来说说，TCP Fast Open 功能的工作方式。

![开启 TCP Fast Open 功能](../source/img/22.jpg)开启 TCP Fast Open 功能
...
![TCP option 字段 - TFO](../source/img/TCP option字段 - TFO.png)TCP option 字段 - TFO
...
![三次握手优化策略](../source/img/24.jpg)三次握手优化策略
...
![客户端主动关闭](../source/img/25.jpg)客户端主动关闭
...
![接收到历史数据的异常](../source/img/31.jpg)接收到历史数据的异常
...
![没有确保正常断开的异常](../source/img/32.jpg)没有确保正常断开的异常
...
![TCP option 字段 - 时间戳](../source/img/TCP option字段-时间戳.png)TCP option 字段 - 时间戳

我们来看看开启了 tcp_tw_reuse 功能，如果四次挥手中的最后一次 ACK 在网络中丢失了，会发生什么？

![四次挥手中的最后一次 ACK 在网络中丢失](../source/img/32.jpg)四次挥手中的最后一次 ACK 在网络中丢失
...
![img](../source/img/36.jpg)

另外，我们可以在程序中设置 socket 选项，来设置调用 close 关闭连接行为。

![img](../source/img/37.jpg)
...
![四次挥手的优化策略](../source/img/39.jpg)四次挥手的优化策略
...
![按数据包进行确认应答](../source/img/40.jpg)按数据包进行确认应答

所以，这样的传输方式有一个缺点：数据包的**往返时间越长，通信的效率就越低**。

**要解决这一问题不难，并行批量发送报文，再批量确认报文即可。**

![并行处理](../source/img/41.jpg)并行处理
...
![TCP 头部](../source/img/42.jpg)TCP 头部
...
![TCP option 选项 - 窗口扩展](../source/img/TCP option字段-窗口.png)TCP option 选项 - 窗口扩展

Linux 中打开这一功能，需要把 tcp_window_scaling 配置设为 1（默认打开）：

![img](../source/img/43.jpg)
...
![数据传输的优化策略](../source/img/49.jpg)数据传输的优化策略
